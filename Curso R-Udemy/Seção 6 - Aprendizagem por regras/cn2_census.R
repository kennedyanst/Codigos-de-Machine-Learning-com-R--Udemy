...
dt_teste <- SF.asDecisionTable(dataset = base_treinamento, decision.attr = 15)

intervalos <- D.discretization.RST(dt_treinamento, nOfIntervals = 4) #Transforma atributos númericos em categoricos

dt_treinamento <- SF.applyDecTable(dt_treinamento, intervalos)
dt_teste <- SF.applyDecTable(dt_teste, intervalos)
classificador <- RI.CN2Rules.RST(dt_treinamento, K = 1) # Vai dá erro pq ele não aceita atributos numericos e sim categoricos. Então não pode rodar o escalonamento.
print(classificador)

previsoes <- predict(classificador, newdata = dt_teste[-15]) #A quantidade de dados é muito grande, demora mais ou menos uns 30min para rodar. 
...
#ZeroR - base line classifier (linha base) = Mostra o valor minimo de acertos que deve ter o algoritimo de Machine Learning
#table(base_teste$income) Pega a quantidade maior de registros e divide pelos registos totais. A porcentagem dirá o valor minimo que o algoritmo de ML deverá ter para ser considerado válido a utilização.
